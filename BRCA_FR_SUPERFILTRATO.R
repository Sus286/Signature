...
presenza_CLINSIG<-grepl("CLIN_SIG",out_brca_fr_filtro_id_vep_clinsig_af$V14)
...
out_brca_fr_completed_filtro<-subset(out_brca_fr_completed,(AF<0.01)|is.na(AF))

# Rimuovi le righe in cui colonna1 è "valore1" o "valore2" (ma mantieni gli NA)
out_brca_fr_completed_filtro <- subset(out_brca_fr_completed_filtro, !(CLINSIG %in% c("likely_benign", "benign","benign,likely_benign","benign/likely_benign"
                                                                                     ,"likely_benign,uncertain_significance","uncertain_significance,likely_benign" )) | is.na(CLINSIG))
...
# Imposta il percorso e il nome del file di output
file_path <- "C:/Users/susan/OneDrive/Desktop/Tesi/BRCA_FR_AF_superFILTRO_sample_inputmatrix.txt"
