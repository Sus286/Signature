...
presenza_AF <- grepl("AF", out_UTCA_US_FILTRO1_id_vep$V14)

out_UTCA_US_FILTRO1_id_vep_clinsig_af<-out_UTCA_US_FILTRO1_id_vep
presenza_CLINSIG<-grepl("CLIN_SIG",out_UTCA_US_FILTRO1_id_vep_clinsig_af$V14)
#out_brca_fr_filtro_id_vep_clinsig_af<-out_brca_fr_filtro_id_vep_clinsig_af[presenza_CLINSIG==TRUE,]

soglia<-out_UTCA_US_FILTRO1_id_vep_clinsig_af[,14]
...
dati_divisi_soglia <- data.frame(matrix(NA, nrow = nrow(out_UTCA_US_FILTRO1_id_vep_clinsig_af), ncol = max_colonne))
...
out_UTCA_US_SOGLIA<-cbind(out_UTCA_US_FILTRO1_id_vep_clinsig_af,parte_numerica_AF)
out_UTCA_US_completed<-cbind(out_UTCA_US_SOGLIA,parte_numerica_CLINSIG)

colnames(out_UTCA_US_completed)[15]<-"AF"
colnames(out_UTCA_US_completed)[16]<-"CLINSIG"

out_UTCA_US_completed_filtro<-subset(out_UTCA_US_completed,(AF<0.01)|is.na(AF))

# Rimuovi le righe in cui colonna1 è "valore1" o "valore2" (ma mantieni gli NA)
out_UTCA_US_completed_filtro <- subset(out_UTCA_US_completed_filtro, !(CLINSIG %in% c("risk_factor,pathogenic,likely_benign","uncertain_significance,conflicting_interpretations_of_pathogenicity,likely_benign","benign,conflicting_interpretations_of_pathogenicity","uncertain_significance,benign/likely_benign","likely_benign", "benign","benign,likely_benign","benign/likely_benign"
...
UTCA_US_CUT_completed <- UTCA_US_FILTRO1_ID[which(UTCA_US_FILTRO1_ID$icgc_mutation_id.1 %in% unique(out_UTCA_US_completed_filtro$V1)),]

#PACA_AU_completed_inputmatrix<-PACA_AU_CUT_completed[,c(3,5,43,13,14,9,10,11,15,17,18)]

UTCA_US_completed_sample_inputmatrix<-UTCA_US_CUT_completed[,c(3,5,5,13,14,9,10,11,15,17,18)]

UTCA_US_completed_sample_inputmatrix<-UTCA_US_completed_sample_inputmatrix[which(UTCA_US_completed_sample_inputmatrix$mutation_type=="single base substitution"),]

#BRCA_FR_completed_inputmatrix[,10]="SOMATIC"

#BRCA_FR_completed_inputmatrix[,4]="SNP"
UTCA_US_completed_sample_inputmatrix[,11]="SOMATIC"

UTCA_US_completed_sample_inputmatrix[,5]="SNP"
...
colnames(UTCA_US_completed_sample_inputmatrix)[1]<-"Project"
colnames(UTCA_US_completed_sample_inputmatrix)[2]<-"Sample"
colnames(UTCA_US_completed_sample_inputmatrix)[3]<-"ID"
colnames(UTCA_US_completed_sample_inputmatrix)[4]<-"Genome"
colnames(UTCA_US_completed_sample_inputmatrix)[5]<-"mut_type"
colnames(UTCA_US_completed_sample_inputmatrix)[6]<-"chrom"
colnames(UTCA_US_completed_sample_inputmatrix)[7]<-"pos_start"
colnames(UTCA_US_completed_sample_inputmatrix)[8]<-"pos_end"
colnames(UTCA_US_completed_sample_inputmatrix)[9]<-"ref"
colnames(UTCA_US_completed_sample_inputmatrix)[10]<-"alt"
colnames(UTCA_US_completed_sample_inputmatrix)[11]<-"Type"
...
# Imposta il percorso e il nome del file di output
file_path <- "C:/Users/susan/OneDrive/Desktop/Tesi/UTCA_US_AF_superFILTRO_sample_inputmatrix.txt"

# Trasforma il dataframe in un vettore di righe di testo
lines <- apply(UTCA_US_completed_sample_inputmatrix, 1, paste, collapse = "\t")

# Scrivi le righe di testo nel file di testo utilizzando cat()
cat(c(paste(names(UTCA_US_completed_sample_inputmatrix), collapse = "\t"), lines), file = file_path, sep = "\n")
