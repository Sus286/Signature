...
presenza_AF <- grepl("AF", out_MALY_DE_FILTRO1_id_vep$V14)

out_MALY_DE_FILTRO1_id_vep_clinsig_af<-out_MALY_DE_FILTRO1_id_vep
presenza_CLINSIG<-grepl("CLIN_SIG",out_MALY_DE_FILTRO1_id_vep_clinsig_af$V14)
#out_brca_fr_filtro_id_vep_clinsig_af<-out_brca_fr_filtro_id_vep_clinsig_af[presenza_CLINSIG==TRUE,]

soglia<-out_MALY_DE_FILTRO1_id_vep_clinsig_af[,14]
...
dati_divisi_soglia <- data.frame(matrix(NA, nrow = nrow(out_MALY_DE_FILTRO1_id_vep_clinsig_af), ncol = max_colonne))
...
out_MALY_DE_SOGLIA<-cbind(out_MALY_DE_FILTRO1_id_vep_clinsig_af,parte_numerica_AF)
out_MALY_DE_completed<-cbind(out_MALY_DE_SOGLIA,parte_numerica_CLINSIG)

colnames(out_MALY_DE_completed)[15]<-"AF"
colnames(out_MALY_DE_completed)[16]<-"CLINSIG"

out_MALY_DE_completed_filtro<-subset(out_MALY_DE_completed,(AF<0.01)|is.na(AF))

# Rimuovi le righe in cui colonna1 è "valore1" o "valore2" (ma mantieni gli NA)
out_MALY_DE_completed_filtro <- subset(out_MALY_DE_completed_filtro, !(CLINSIG %in% c("likely_benign,conflicting_interpretations_of_pathogenicity","benign,uncertain_significance","uncertain_significance,benign,conflicting_interpretations_of_pathogenicity,likely_pathogenic","benign,benign/likely_benign","uncertain_significance,benign,benign/likely_benign,likely_benign","conflicting_interpretations_of_pathogenicity,likely_benign","benign,likely_benign,risk_factor","uncertain_significance,benign,pathogenic","benign,likely_benign,uncertain_significance,conflicting_interpretations_of_pathogenicity","uncertain_significance,conflicting_interpretations_of_pathogenicity,likely_benign","benign,conflicting_interpretations_of_pathogenicity","uncertain_significance,benign/likely_benign","likely_benign", "benign","benign,likely_benign","benign/likely_benign"
...
MALY_DE_CUT_completed <- MALY_DE_FILTRO1_ID[which(MALY_DE_FILTRO1_ID$icgc_mutation_id.1 %in% unique(out_MALY_DE_completed_filtro$V1)),]

#PACA_AU_completed_inputmatrix<-PACA_AU_CUT_completed[,c(3,5,43,13,14,9,10,11,15,17,18)]

MALY_DE_completed_sample_inputmatrix<-MALY_DE_CUT_completed[,c(3,5,5,13,14,9,10,11,15,17,18)]

MALY_DE_completed_sample_inputmatrix<-MALY_DE_completed_sample_inputmatrix[which(MALY_DE_completed_sample_inputmatrix$mutation_type=="single base substitution"),]

#BRCA_FR_completed_inputmatrix[,10]="SOMATIC"

#BRCA_FR_completed_inputmatrix[,4]="SNP"
MALY_DE_completed_sample_inputmatrix[,11]="SOMATIC"

MALY_DE_completed_sample_inputmatrix[,5]="SNP"
...
colnames(MALY_DE_completed_sample_inputmatrix)[1]<-"Project"
colnames(MALY_DE_completed_sample_inputmatrix)[2]<-"Sample"
colnames(MALY_DE_completed_sample_inputmatrix)[3]<-"ID"
colnames(MALY_DE_completed_sample_inputmatrix)[4]<-"Genome"
colnames(MALY_DE_completed_sample_inputmatrix)[5]<-"mut_type"
colnames(MALY_DE_completed_sample_inputmatrix)[6]<-"chrom"
colnames(MALY_DE_completed_sample_inputmatrix)[7]<-"pos_start"
colnames(MALY_DE_completed_sample_inputmatrix)[8]<-"pos_end"
colnames(MALY_DE_completed_sample_inputmatrix)[9]<-"ref"
colnames(MALY_DE_completed_sample_inputmatrix)[10]<-"alt"
colnames(MALY_DE_completed_sample_inputmatrix)[11]<-"Type"
...
# Imposta il percorso e il nome del file di output
file_path <- "C:/Users/susan/OneDrive/Desktop/Tesi/MALY_DE_AF_superFILTRO_sample_inputmatrix.txt"

# Trasforma il dataframe in un vettore di righe di testo
lines <- apply(MALY_DE_completed_sample_inputmatrix, 1, paste, collapse = "\t")

# Scrivi le righe di testo nel file di testo utilizzando cat()
cat(c(paste(names(MALY_DE_completed_sample_inputmatrix), collapse = "\t"), lines), file = file_path, sep = "\n")
